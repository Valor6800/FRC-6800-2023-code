...
{ 
    ...
    // STOW POSITION
    stowPos = frc::Pose2d(-0.428_m, 0.406_m, 0.0_deg);

    autoStowPos = frc::Pose2d(-0.428_m, 0.406_m, 45.0_deg);
    
    // FRONT CONE
    posMap[Piece::CONE][Direction::FRONT][Position::GROUND] =frc::Pose2d(0.086_m, 0.4_m, 121.4_deg);
    ...
    // FRONT CUBE
    posMap[Piece::CUBE][Direction::FRONT][Position::GROUND] =frc::Pose2d(0.185_m, 0.409_m, 216.57_deg);
    ...
    posMap[Piece::CUBE][Direction::FRONT][Position::MID] =frc::Pose2d(0.273_m, 1.054_m, -46.93_deg);
    posMap[Piece::CUBE][Direction::FRONT][Position::HIGH] =frc::Pose2d(0.577_m, 1.255_m, 231.25_deg);
    ...
    // BACK CONE   
    posMap[Piece::CONE][Direction::BACK][Position::GROUND] =frc::Pose2d(-0.98_m, 0.622_m, -233.5_deg);
    ...
    // BACK CUBE
    posMap[Piece::CUBE][Direction::BACK][Position::GROUND] =frc::Pose2d(-0.969_m, 0.306_m, -139.13_deg);
...
{   
    ...
    if (operatorGamepad->leftStickYActive() || operatorGamepad->rightStickYActive()) {
        ...
        if (previousState.positionState != Position::MANUAL) {
            if (intake->getFuturePiece() == Piece::CONE) futureState.positionState = Position::SNAKE;
            else futureState.positionState = Position::STOW;
        } 
    }
